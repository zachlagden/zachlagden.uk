...
- [x] **DEP-01**: All 44 Dependabot alerts resolved (19 high + 19 moderate + 6 low); `pnpm audit` shows 0 high/critical and 0 moderate after the milestone closes
...
- [x] **DEP-04**: `pnpm dedupe` run after each batch; lockfile churn audited for unintended transitive duplicates
- [x] **DEP-05**: Per-batch verification floor enforced: `tsc --noEmit && pnpm lint && pnpm build` must pass before each Dependabot batch is merged; failures roll back the batch
